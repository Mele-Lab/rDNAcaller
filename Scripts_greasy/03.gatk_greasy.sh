...
TMPDIR=/gpfs/projects/bsc83/Projects/ribosomal_RNAs/Jose/04_Pipeline/temporal/ #Using greasy the default TMPDIR might not work correctly

if [[ $type == "BP_RESOLUTION" ]]; then
...
rm ${TMPDIR}/${sample}.${ploidy}.g.vcf.gz ${TMPDIR}/${sample}.${ploidy}_filtering_1.g.vcf.gz ${TMPDIR}/${sample}.${ploidy}_filtering.g.vcf.gz ${TMPDIR}/${sample}.${ploidy}_filtering.g_wo_PL.vcf ${TMPDIR}/${sample}.${ploidy}_trim.vcf.gz 
